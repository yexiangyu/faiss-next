...
#[cfg(feature = "gpu")]
use faiss::gpu::cloner_options::{GpuClonerOptions, GpuMultipleClonerOptions};
#[cfg(feature = "gpu")]
...
fn main() -> Result<()> {
    ...
    let num_gpu = faiss::gpu::device_utils::get_num_gpus()?;
    if num_gpu < 2 {
        // error!("not enough gpu devices");
        // return Ok(());
    }

    let options = GpuMultipleClonerOptions::new()?
        .enable_shard()
        .with_shard_type(0);
    ...
    let mut index = {
        ...
        IndexGpuImpl::new(
            vec![StandardGpuResources::new()?, StandardGpuResources::new()?],
            vec![0, 0],
            &index,
            Some(options),
        )?
